##########################################################################################################
##########################################################################################################
############## Simulation for variable selection (p=10, n=200, EX2 independent covariates)
##########################################################################################################
##########################################################################################################
...
h_grid <- c(0.19,0.21,0.23,0.25,0.27)
k_fold <- 10
#set.seed(248)
...
sim_spmixreg_VS <- function(i){
  ...
  while(out_acSCAD$iter==1000 & l_ini < 10) {
    ...
    initl_beta1_new <- beta_1 + rnorm(p,sd=0.5)
    initl_beta2_new <- beta_2 + rnorm(p,sd=0.5)
    out_acSCAD <- semipnormmixEM_SCAD(z=z,x=xd,y,pi1z,pi2z,initl_beta1_new,initl_beta2_new,initl_sgm21,initl_sgm22,gama1=optgama1_L1,gama2=optgama2_L1,maxiter=1000,epsilon=0.001,bw=h_opt)
  }
